@php
    $approvalLocked = !empty($editPengajuan) && (empty($canEdit) || (int) ($editPengajuan->is_draft ?? 0) === 1);
...
<div class="page-header">
    <div>
        <h4 class="page-title">
            Pengajuan Pembelian Purchasing
        </h4>
    ...
    <div style="display:flex;gap:8px;align-items:center;">
        <button type="button" class="btn btn-primary btn-sm" id="btnSaveRequest"
        ...
        <button type="button" class="btn btn-outline-danger btn-sm" id="btnClearCache"
            {{ !empty($editPengajuan) && empty($canEdit) ? 'disabled' : '' }}>
...
<div class="request-info">
    ...
    <div class="info-box">
        ...
        <input type="date" id="requestDate" value="{{ date('Y-m-d') }}">
    </div>
...
<div id="warehouseSearchSection" class="search-section" @if ($isViewerOnly) style="display:none;" @endif>
    ...
    <div class="search-box">
        ...
        <input type="text" id="materialSearch" autocomplete="off"
            placeholder="Cari kode atau nama barang di gudang...">

        <button type="button" class="search-button" id="btnSearchMaterial">
            <i class="fa fa-search"></i>
        </button>
    ...
    <div id="selectedMaterial" class="selected-material">
        ...
        <div class="selected-header">
            ...
            <div>
                <div class="selected-name" id="selectedMaterialName">
                    -
                </div>

                <div class="selected-code" id="selectedMaterialCode">
                    -
                </div>
            </div>

            <span class="badge bg-info" id="selectedUnit">
                -
            </span>
        ...
        <div class="stock-info">
            ...
            <div class="stock-box">
                ...
                <div class="stock-value" id="warehouseStock">
                    0
                </div>
            </div>
            ...
            <div class="stock-box">
                ...
                <div class="stock-value" id="stockStatus">
                    Belum dicek
                </div>
            </div>
        ...
        <div class="request-input">
            ...
            <div>
                ...
                <input type="number" min="0.01" step="0.01" id="requestQty" class="form-control-custom"
            ...
            <div>
                ...
                <select id="requestReason" class="form-select-custom">
                    <option value="">
                        -- Pilih alasan --
                    </option>
            ...
            <button type="button" class="btn-add-material" id="btnAddMaterial">
    ...
    <div id="newMaterialForm" class="new-material-form">
        ...
        <div class="new-material-grid">
            <div>
                ...
                <input type="text" id="newMaterialName" class="form-control-custom"
                    placeholder="Nama barang yang akan dibeli">
            </div>

            <div>
                <label class="form-label-custom">Satuan *</label>
                <input type="text" id="newMaterialUnit" class="form-control-custom"
                    placeholder="Pcs / Kg / Roll / dll">
            </div>

            <div>
                <label class="form-label-custom">Qty Kebutuhan *</label>
                <input type="number" id="newMaterialQty" min="0.01" step="0.01" class="form-control-custom"
                    placeholder="Qty">
            </div>
        </div>

        <div class="new-material-actions">
            <button type="button" class="btn-new-material" id="btnAddNewMaterial">
                <i class="fa fa-plus"></i>
                Tambah Barang Baru
            </button>

            <button type="button" class="btn-cancel-new-material" id="btnCancelNewMaterial">
                Batal
            </button>
        </div>
    </div>
...
<div id="purchasingFormSection" class="table-section">
    ...
    <div class="table-footer">
        ...
        <button type="button" class="btn-add-row" id="btnAddManualRow"
            @if ($isViewerOnly) style="display:none;" @endif>
            <i class="fa fa-plus"></i>
            Add Row
        </button>

        <button type="button" class="btn btn-success btn-sm" id="btnSubmitRequest">
    ...
    @if ($isViewerOnly)
        <div id="viewerPurchaseRequest" class="viewer-purchase-request">
            ...
            <div class="vpr-top">
                <div class="vpr-brand">
                    <img src="{{ asset('assets/images/newwicker-logo.png') }}" alt="NewWicker" class="vpr-logo">
                </div>
                <div class="vpr-title">Purchase Request</div>
                <div class="vpr-need">
                    <b>Need by Date :</b>
                    <span>
                        {{ !empty($editData['need_date']) ? \Carbon\Carbon::parse($editData['need_date'])->format('j-M-y') : '-' }}
                    </span>
                </div>
            </div>
            ...
            <div class="vpr-meta">
                ...
                <div>
                    <b>Department :</b>
                    <span>
                        {{ optional($editPengajuan->divisi)->nama ?? (optional($editPengajuan->divisi)->name ?? '-') }}
                    </span>
                </div>
    ...
    <div class="attachment-section" id="attachmentSection" style="display:none;">
        ...
        <div id="attachmentCreator" style="display:none;">
            <input type="file" id="attachmentInput" name="images[]" accept="image/*" multiple
                style="display:none;">
        ...
        <div id="attachmentViewer" style="display:none;">
            ...
            <div id="attachmentViewerEmpty" style="display:none;text-align:center;color:#999;padding:20px;">
                <i class="fa fa-image" style="font-size:28px;display:block;margin-bottom:7px;"></i>
        ...
        <table class="signature-table">
            ...
            <tbody>
                ...
                <tr class="signature-input-row">
                    ...
                    {{-- MADE BY --}}
                    <td>
                        <input type="text" value="{{ $madeByName }}" readonly class="signature-input">
                    </td>
...
<script>
    $(document).ready(function() {
            ...
            Object.keys(fieldMap).forEach(function(order) {
                ...
                if (assignedUserId === CURRENT_USER_ID) {
                    ...
                    cell.append(
                        '<div class="approval-tap-wrap">' +
                        '<button type="button" ' +
                        'class="approval-tap-btn approval-tap-not-authorized" ' +
                        'data-step-order="' + order + '" ' +
                        'data-step-name="' + escapeHtml(assignedUserName) + '">' +
                        '<i class="fa fa-signature"></i> Tanda Tangan' +
                        '</button>' +
        ...
        $(document).on('click.purchasingApproval', '.approval-tap-btn', function(e) {
            ...
            const currentStep = APPROVAL_STEPS.find(function(item) {
                return String(item.step_order) === String(stepOrder);
            });

            const assignedUserId = Number(currentStep && currentStep.user_id || 0);

            if (assignedUserId !== CURRENT_USER_ID) {
                alert(
                    'Anda bukan approver untuk tanda tangan ini.\n\n' +
                    'Tanda tangan ini hanya dapat dilakukan oleh: ' + stepName
                );
                return;
            }
        ...
        $(document).on('click.purchasingSaveDraft', '#btnSaveRequest', function(e) {
            ...
            formData.append(
                ...
                EDIT_MODE && EDIT_DATA ?
                EDIT_DATA.id :
                (localStorage.getItem('pengajuan_purchasing_id') || '')
            );
            formData.append('tanggal', $('#requestDate').val() || '');
            formData.append('divisi_id', $('#department').val() || '');
            formData.append('need_date', $('#neededDate').val() || '');

            requestItems.forEach(function(item, index) {
                formData.append(`items[${index}][id_stock]`, item.id || '');
                formData.append(`items[${index}][nama_barang]`, item.name || '');
                formData.append(`items[${index}][supplier]`, item.supplier || '');
                formData.append(`items[${index}][po_no]`, item.po_no || '');
                formData.append(`items[${index}][payment]`, item.payment || '');
            ...
            formData.append('signature[checked_by_2]', $('#checked_by_2').val() || '');
            formData.append('signature[checked_by_3]', $('#checked_by_3').val() || '');
            formData.append('signature[checked_by_4]', $('#checked_by_4').val() || '');
            formData.append('signature[checked_by_finance]', $('#checked_by_finance').val() || '');
            formData.append('signature[approved_by]', $('#approved_by').val() || '');
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        showCacheStatus(
                            EDIT_MODE ? '✓ Perubahan berhasil disimpan ke database.' :
                            '✓ Draft berhasil disimpan ke database.',
                            'saved'
                        );

                        alert(EDIT_MODE ? 'Perubahan pengajuan berhasil disimpan.' :
                ...
                error: function(xhr) {
                    ...
                    if (xhr.responseJSON && xhr.responseJSON.message &&
                        String(xhr.responseJSON.message).toLowerCase().includes(
                            'no query results for model')) {
                        message =
                            'Draft database sebelumnya tidak ditemukan. ID draft browser tetap dipertahankan agar data cache tidak hilang. Silakan cek database sebelum membuat draft baru.';
                    }
                ...
                complete: function() {
                    ...
                    button
                        .prop('disabled', false)
                        .html('<i class="fa fa-save"></i> ' + (EDIT_MODE ?
                            'Simpan Perubahan' : 'Simpan Draft'));
                }
            });
        });
        ...
        /*
         * Data attachment existing dari Laravel.
         * Dibuat GLOBAL melalui window supaya bisa dicek langsung dari
         * Chrome DevTools Console.
         */
        window.PURCHASING_ATTACHMENT_DEBUG = window.PURCHASING_ATTACHMENT_DEBUG || {};

        window.PURCHASING_ATTACHMENT_DEBUG.editMode = EDIT_MODE;
        window.PURCHASING_ATTACHMENT_DEBUG.canEdit = CAN_EDIT;
        window.PURCHASING_ATTACHMENT_DEBUG.editData = EDIT_DATA;

        @php
            $existingAttachmentDebug = [];
            if (!empty($editPengajuan) && isset($editPengajuan->files)) {
                $existingAttachmentDebug = $editPengajuan->files
                    ->where('type', 'image')
                    ->map(function ($file) {
        ...
        const EXISTING_ATTACHMENT_FILES = @json($existingAttachmentDebug);

        window.PURCHASING_ATTACHMENT_DEBUG.existing = EXISTING_ATTACHMENT_FILES;

        console.groupCollapsed('%c[PURCHASING ATTACHMENT] INIT', 'font-weight:bold');
        console.log('EDIT_MODE:', EDIT_MODE);
        console.log('CAN_EDIT:', CAN_EDIT);
        console.log('EDIT_DATA:', EDIT_DATA);
        console.log('EXISTING_ATTACHMENT_FILES:', EXISTING_ATTACHMENT_FILES);
        console.log('Attachment count dari Blade:', EXISTING_ATTACHMENT_FILES.length);
        console.log('DOM #attachmentViewer:', document.getElementById('attachmentViewer'));
        console.log('DOM #attachmentViewerList:', document.getElementById('attachmentViewerList'));
        console.groupEnd();
        ...
        function attachmentUrl(path) {
            ...
            // Hanya gunakan path file dari database.
            // Jangan gunakan URL hasil Storage::disk('public')->url()
        ...
        window.PURCHASING_ATTACHMENT_DEBUG.attachmentUrl = attachmentUrl;

        function renderAttachmentPreview() {
            const container = $('#attachmentPreview');
            container.empty();

            console.groupCollapsed('%c[PURCHASING ATTACHMENT] PREVIEW', 'font-weight:bold');
            console.log('File baru dipilih:', attachmentFiles.length);
            console.groupEnd();
        ...
        function renderExistingAttachments() {
            ...
            console.group('%c[PURCHASING ATTACHMENT] RENDER EXISTING', 'font-weight:bold');
            console.log('Jumlah data attachment:', EXISTING_ATTACHMENT_FILES.length);
            console.log('Data attachment:', EXISTING_ATTACHMENT_FILES);
            console.log('Viewer element:', container.length ? container[0] : null);

            if (!EXISTING_ATTACHMENT_FILES || EXISTING_ATTACHMENT_FILES.length === 0) {
                console.warn('Tidak ada attachment existing yang diterima dari Laravel.');
            ...
            EXISTING_ATTACHMENT_FILES.forEach(function(file, index) {
                ...
                if (!url) {
                    console.error(`Attachment ${index + 1} TIDAK memiliki URL.`, file);
                    return;
                }

                const itemHtml = `
                <div class="attachment-viewer-item" data-attachment-id="${file && file.id ? file.id : ''}">
            ...
            `;
                ...
                const img = container.find('img').last()[0];

                if (img) {
                    img.addEventListener('load', function() {
                        console.log(`%cAttachment ${index + 1} LOAD OK`,
                    ...
                    img.addEventListener('error', function() {
                        console.error(`%cAttachment ${index + 1} LOAD ERROR`,
                        ...
                        this.closest('.attachment-viewer-item').style.display = 'block';
                        this.style.display = 'none';

                        const errorLabel = document.createElement('div');
                        errorLabel.style.cssText =
                            'padding:15px;text-align:center;color:#dc3545;background:#fff5f5;border:1px solid #f5c2c7;border-radius:6px;font-size:12px;';
                        errorLabel.innerHTML = '<b>Gagal memuat gambar</b><br><small>' + this
                            .src + '</small>';
                        this.closest('.attachment-viewer-item').appendChild(errorLabel);
                    });
                }
            });

            console.log('Jumlah IMG setelah render:', container.find('img').length);
            console.groupEnd();
        }

        window.PURCHASING_ATTACHMENT_DEBUG.renderExistingAttachments = renderExistingAttachments;

        // ============================================================
        // ATTACHMENT VIEWER - AUTO LOAD
        // ============================================================
        if (EDIT_MODE) {
            console.log('%c[PURCHASING ATTACHMENT] EDIT MODE - menyiapkan viewer', 'font-weight:bold');
        ...
        function openAttachmentSection() {
            console.log('[PURCHASING ATTACHMENT] openAttachmentSection()');

            $('#attachmentSection').slideToggle(180, function() {
                console.log('[PURCHASING ATTACHMENT] section visible:', $('#attachmentSection').is(
                    ':visible'));
            });
        ...
        $(document).on('click.purchasingAttachment', '#btnSubmitRequest', function(e) {
            ...
            console.group('%c[PURCHASING ATTACHMENT] BUTTON CLICK', 'font-weight:bold');
            console.log('CAN_EDIT:', CAN_EDIT);
            console.log('Existing attachment:', EXISTING_ATTACHMENT_FILES);
            console.log('Jumlah existing:', EXISTING_ATTACHMENT_FILES.length);
            console.log('Section sebelum:', $('#attachmentSection').is(':visible'));
            console.groupEnd();
        ...
        if (EDIT_MODE && !CAN_EDIT) {
            ...
            $('#btnAddMaterial, #btnAddNewMaterial, #btnCancelNewMaterial, #btnAddManualRow').prop('disabled',
                true);
        ...
        // ============================================================
        $(document).off('click.warehouseAdd', '.btn-add-to-warehouse');

        $(document).on('click.warehouseAdd', '.btn-add-to-warehouse', function(e) {
            e.preventDefault();
            e.stopPropagation();

            const button = $(this);
            const itemId = button.data('item-id');
            const itemName = button.data('item-name') || 'Barang';
            const qty = button.data('qty') || '0';
            const unit = button.data('unit') || '-';

            if (!itemId) {
                alert('ID detail barang tidak ditemukan.');
                return;
            }
            ...
            if (!confirm(
                    ...
                    'Barang: ' + itemName + '\n' +
                    'Qty: ' + qty + ' ' + unit + '\n\n' +
                    'Barang akan dicatat sebagai transaksi IN ke warehouse.\n\n' +
                    'Lanjutkan?'
                )) {
                return;
            }
            ...
            $.ajax({
                url: "{{ url('/pengajuan_purchasing/detail') }}/" + itemId +
                    "/add-to-warehouse",
                type: 'POST',
                dataType: 'json',
                data: {
                ...
                success: function(response) {
                    if (!response.success) {
                        alert(response.message || 'Gagal menambahkan barang ke warehouse.');
                        button
                            .prop('disabled', false)
                            .html('<i class="fa fa-plus-circle"></i> Add to Warehouse');
                        return;
                    }

                    alert(response.message || 'Barang berhasil ditambahkan ke warehouse.');

                    button
                        .prop('disabled', true)
                        .html('<i class="fa fa-check-circle"></i> Added to Warehouse');
                },
                error: function(xhr) {
                    console.error('ADD TO WAREHOUSE ERROR:', xhr.responseJSON || xhr
                        .responseText);

                    let message = 'Gagal menambahkan barang ke warehouse.';
                    if (xhr.responseJSON && xhr.responseJSON.message) {
                        message = xhr.responseJSON.message;
                    }
...
<style>
    ...
    .btn-add-to-warehouse:hover {
        background: #198754;
        color: #fff;
    }

    .btn-add-to-warehouse i {
        margin-right: 3px;
    }
    ...
    .approval-tap-btn.approval-tap-not-authorized {
        background: #6c757d;
        cursor: pointer;
    }

    .approval-tap-btn.approval-tap-not-authorized:hover {
        opacity: .9;
    }
...
<div id="purchasingFloatingNav" class="purchasing-floating-nav">
    ...
    <div class="purchasing-floating-body">
        <button type="button" class="purchasing-nav-btn" data-scroll-target="#purchasingFormSection">
            <i class="fa fa-list"></i>
            <span>List Form</span>
        </button>

        <button type="button" class="purchasing-nav-btn" data-scroll-target="#purchasingSignatureSection">
