...
class SpkController extends Controller
{
    ...
    {
            $viewOnly = $request->is('spk/view/*');

        $mode = $request->routeIs('spk.edit')
            ? 'edit'
            : 'create';
        $jenisSuppliers =
    ...
    */
    if ($request->type == 'checked') {
        ...
        if (auth()->id() != 171) {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Hanya Vivi yang dapat melakukan approval ini'
            ], 403);
        }
