                    ...
                    foreach ($items as $item) {
                        ...
                        if (
                            ...
                            $relatedSpks[] = [
                                ...
                                'supplier' => $spkData['sup'] ?? null,

                                'kategori' => $spkData['kategori'] ?? null,

                                'status' => $spkData['status'] ?? null,

                                'no_spk' => $spkData['no_spk'] ?? null,

                                'tgl_terima' => $spkData['tgl_terima'] ?? null,

                                'tgl_selesai' => $spkData['tgl_selesai'] ?? null,
...
*/
    ...
    // export
    public function exportPass(Request $request)
    {
        ...
        // filter inspector
        if ($request->filled('inspector')) {
            $inspection->where('user_id', $request->inspector);
        }

        // filter tanggal
        if ($request->filled('from') && $request->filled('to')) {
            $inspection->whereBetween('tanggal_inspect', [
                $request->from,
                $request->to
            ]);
        }
        ...
        return Excel::download(
            ...
            'QC_PASS_' . $request->from . '_' . $request->to . '.xlsx'
        );
    }
}
